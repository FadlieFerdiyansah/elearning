...
class Mahasiswa extends Authenticatable
{
    ...
    public function mahasiswaAbsenHariIniTidakHadir()
    {
            return $this->hasOne(Absen::class)
            ->whereNotNull('mahasiswa_id')
            ->where('parent','!=', 0)
            ->where('status', '0')->orWhere('status', null)
            ->whereDate('created_at', date('Y-m-d'));
    }

    public function mahasiswaAbsenHariIniIzin()
    {
            return $this->hasOne(Absen::class)
            ->whereNotNull('mahasiswa_id')
            ->where('parent','!=', 0)
    ...
    public function mahasiswaAbsenHariIniSakit()
    {
            return $this->hasOne(Absen::class)
            ->whereNotNull('mahasiswa_id')
            ->where('parent','!=', 0)
